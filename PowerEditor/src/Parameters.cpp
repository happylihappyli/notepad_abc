		...
		if (viewRoots[k])
		{
				...
				childNode = childNode->NextSibling(L"File") )
			{
				...
				if (fileName)
				{
					...
					// C++17 compatible prefix check (replacing C++20's starts_with)
					if (backupFilePath.length() < currentBackupFilePath.length() || 
						backupFilePath.compare(0, currentBackupFilePath.length(), currentBackupFilePath) != 0)
					{
