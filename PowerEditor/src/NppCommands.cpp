	...
	case IDM_VIEW_PROJECT_PANEL_1:
		...
		case IDM_VIEW_SWITCHTO_FUNC_LIST:
		{
			...
			else
			{
				...
				debugLog(L"调用launchFunctionList()\n");
				launchFunctionList();
				if (_pFuncList)
				{
					debugLog(L"函数列表面板创建成功\n");
					_pFuncList->setClosed(false);
				}
				else
				{
					debugLog(L"函数列表面板创建失败\n");
				}
			}
		}
		break;
		...
		{
			debugLog(L"开始处理函数列表命令\n");
			wchar_t buffer[256];
		swprintf(buffer, 256, L"_pFuncList 指针状态: %p\n", _pFuncList);
		debugLog(buffer);
			if (_pFuncList)
			{
				wchar_t buffer[256];
		swprintf(buffer, 256, L"函数列表面板已存在，检查是否关闭: %d\n", _pFuncList->isClosed());
		debugLog(buffer);
				if (!_pFuncList->isClosed())
				{
					debugLog(L"隐藏函数列表面板\n");
				...
				{
					debugLog(L"准备显示函数列表面板\n");
					checkMenuItem(IDM_VIEW_FUNC_LIST, true);
					_toolBar.setCheck(IDM_VIEW_FUNC_LIST, true);
					debugLog(L"调用launchFunctionList()\n");
					launchFunctionList();
					wchar_t buffer[256];
		swprintf(buffer, 256, L"检查_pFuncList指针是否有效: %p\n", _pFuncList);
		debugLog(buffer);
					if (_pFuncList)
					{
						debugLog(L"函数列表面板创建成功\n");
					...
					{
						debugLog(L"错误: launchFunctionList()后_pFuncList指针无效\n");
					}
				}
			}
			else
			{
				debugLog(L"函数列表面板不存在，准备创建\n");
				checkMenuItem(IDM_VIEW_FUNC_LIST, true);
				_toolBar.setCheck(IDM_VIEW_FUNC_LIST, true);
				debugLog(L"调用launchFunctionList()\n");
				launchFunctionList();
