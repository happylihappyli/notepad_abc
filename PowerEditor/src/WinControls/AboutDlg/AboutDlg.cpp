...
void AppendDisplayAdaptersInfo(wstring& strOut, const unsigned int maxAdaptersIn)
{
	...
	if ((lStatus == ERROR_SUCCESS) && (dwSubkeysCount > 0))
	{
		...
		for (DWORD i = 0; i < dwSubkeysCount; ++i)
		{
			...
			// C++17 compatible formatting (replacing C++20's std::format)
			wstring strAdapterNo = L"000" + std::to_wstring(i);
			if (i >= 10) strAdapterNo = L"00" + std::to_wstring(i);
			if (i >= 100) strAdapterNo = L"0" + std::to_wstring(i);
			if (i >= 1000) strAdapterNo = std::to_wstring(i); // 0000, 0001, 0002, etc...
...
intptr_t CALLBACK AboutDlg::run_dlgProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
		case WM_INITDIALOG:
		{
			...
			_pageLink.create(::GetDlgItem(_hSelf, IDC_AUTHOR_NAME), L"https://www.funnyai.com/");
