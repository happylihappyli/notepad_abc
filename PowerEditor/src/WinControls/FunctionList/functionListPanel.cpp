...
// 实现FunctionListPanel的create方法
void FunctionListPanel::create(tTbData* data, bool isRTL) {
	// 使用基类的create方法，它会从资源文件加载对话框
	DockingDlgInterface::create(data, isRTL);

	// 初始化TreeView对象
	_treeView.Window::init(_hInst, _hSelf);
	_treeViewSearchResult.Window::init(_hInst, _hSelf);
}

void FunctionListPanel::create(tTbData* data, std::array<int, 3> iconIDs, bool isRTL) {
	_iconIDs = iconIDs;
	// 调用另一个重载版本的create方法
...
intptr_t CALLBACK FunctionListPanel::run_dlgProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
		...
		{
			// 窗口创建时初始化TreeView控件
			// 先尝试获取对话框中已有的TreeView控件
			HWND hTreeView = ::GetDlgItem(_hSelf, IDC_LIST_FUNCLIST);
			HWND hTreeViewAux = ::GetDlgItem(_hSelf, IDC_LIST_FUNCLIST_AUX);

			// 确保TreeView控件正确初始化
			if (hTreeView && _treeView.getHSelf() != hTreeView)
			{
				_treeView.setHSelf(hTreeView);
			}

			if (hTreeViewAux && _treeViewSearchResult.getHSelf() != hTreeViewAux)
			{
				_treeViewSearchResult.setHSelf(hTreeViewAux);
			}
