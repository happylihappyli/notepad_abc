...
private:
	...
	bool changeExt(wstring& name, int extIndex)
	{
		...
		const std::wstring wildcardSuffix = L".*";
		if (ext.length() < wildcardSuffix.length() || 
			ext.compare(ext.length() - wildcardSuffix.length(), wildcardSuffix.length(), wildcardSuffix) != 0)
