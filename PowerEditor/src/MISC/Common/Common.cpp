...
bool buf2Clipboard(const std::vector<Buffer*>& buffers, bool isFullPath, HWND hwnd)
{
	...
	for (auto&& buf : buffers)
	{
		...
		// C++17 compatible suffix check (replacing C++20's ends_with)
		bool endsWithCrlf = (selection.length() >= crlf.length() && 
			selection.compare(selection.length() - crlf.length(), crlf.length(), crlf) == 0);
		if (!selection.empty() && !endsWithCrlf)
			selection += crlf;
	}
...
bool isWin32NamespacePrefixedFileName(const wstring& fileName)
{
	...
	// C++17 compatible prefix checks (replacing C++20's starts_with)
	const std::wstring prefix1 = L"\\\\?\\";
	const std::wstring prefix2 = L"//?/";
	return ((fileName.length() >= prefix1.length() && fileName.compare(0, prefix1.length(), prefix1) == 0) ||
		(fileName.length() >= prefix2.length() && fileName.compare(0, prefix2.length(), prefix2) == 0));
}
...
bool isUnsupportedFileName(const wstring& fileName)
{
	...
	if ((fileName.size() > 0) && (fileName.size() < MAX_PATH))
	{
		...
		// C++17 compatible suffix checks (replacing C++20's ends_with)
		bool endsWithDot = (fileName.length() >= 1 && fileName[fileName.length() - 1] == L'.');
		bool endsWithSpace = (fileName.length() >= 1 && fileName[fileName.length() - 1] == L' ');
		if (!(endsWithDot && isWin32NamespacePrefixedFileName(fileName)) && !endsWithSpace)
		{
