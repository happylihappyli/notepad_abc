...
void Notepad_plus_Window::init(HINSTANCE hInst, HWND parent, const wchar_t *cmdLine, CmdLineParams *cmdLineParams)
{
	...
	if (cmdLineParams->_showLoadingTime)
	{
		...
		// C++17 compatible time formatting (replacing C++20's std::chrono::hh_mm_ss)
		auto formatDuration = [](std::chrono::milliseconds ms) -> std::wstring {
			...
			std::wstringstream wss;
			wss << std::setfill(L'0') << std::setw(2) << hours.count() << L":"
			    << std::setfill(L'0') << std::setw(2) << minutes.count() << L":"
			    << std::setfill(L'0') << std::setw(2) << seconds.count() << L"."
